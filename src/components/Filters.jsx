...
const Filters = ({ setMainData }) => {
    ...
    const getDataFromDB = async(year) => {
        try{
            const response = await axios.get(`https://dashboard-server-1-h3br.onrender.com/api/data/year/${year}`);
    ...
    const handleReset = async() => {
        try{
            const response = await axios.get("https://dashboard-server-1-h3br.onrender.com/api/data/all");
            setMainData(response.data.data)
        }
        catch(e){
            console.log(e)
